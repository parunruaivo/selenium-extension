...
public final class ProxyFactory {
	...
	public static <T extends WebDriver> TestrigorDriver createDriverProxy(ClassLoader loader, InvocationHandler handler, Class<T> clazz) {
		Class<?>[] interfaces = Stream.concat(
			Arrays.stream(clazz.getInterfaces()),
			Stream.of(JavascriptExecutor.class, TestrigorDriver.class, Interactive.class)
		).distinct().toArray(Class[]::new);

		return (TestrigorDriver) Proxy.newProxyInstance(loader, interfaces, handler);
	}
